...
- **Working-folder mode (`use_working_folder_as_repo`)**: Analyze only current HEAD and its ancestor commits. Before citing any SHA, verify `git merge-base --is-ancestor <sha> HEAD`. Do **not** use futuristic / unreachable commits (later merges, remote tips ahead of HEAD, GitHub PR fix SHAs) as prior art or fix recipes. Design recommendations from the buggy HEAD tree. Exception: user explicitly names a commit/PR to consult. See schema `working_folder_repo.git_history_scope`.
