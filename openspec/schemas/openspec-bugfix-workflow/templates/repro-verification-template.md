...
- **Working-folder mode (`use_working_folder_as_repo`)**: Record `git rev-parse HEAD` as the verification baseline. Inspect only the current tree and commits reachable from HEAD. Do **not** cite or rely on futuristic commits (SHAs not ancestors of HEAD — e.g. later origin/master tips or fix PRs that land after a deliberate reset). See schema `working_folder_repo.git_history_scope`.
