...
- **Working-folder mode:** Plan from the current HEAD tree only. Do not adopt fix designs
  from commits not reachable from HEAD (futuristic SHAs / later merged PRs) unless the user
  explicitly directs consulting them. See schema `working_folder_repo.git_history_scope`.
